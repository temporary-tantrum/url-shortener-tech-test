...
def test_index():
    """
        hello, world
    """
    r = requests.get('http://localhost:8000/', timeout=500)
...
def test_post():
    ...
    r = requests.post('http://localhost:8000/url/shorten',
        data=json.dumps({'url': url_to_shorten}), timeout=500)
...
def test_longen_post():
    ...
    r = requests.post('http://localhost:8000/url/longen',
        data=json.dumps({'url': url_to_longen}), timeout=500)
...
def test_redirect():
    ...
    """
    r = requests.get('http://localhost:8000/r/abc123', timeout=500,
...
def test_that_the_redirect_logic_works_the_way_we_expect_it_to():
    ...
    r = requests.get(short_url, timeout=500, allow_redirects=False)
    assert r.status_code == 307
